...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans: int = 3, num_classes: int = 1000, depths: list = None,
        ...
        self.norm1 = nn.LayerNorm(dims[-1]*2, eps=1e-6)#分类头1的layernorm
        self.norm2 = nn.LayerNorm(dims[-1], eps=1e-6)# 分类头2的layernrom
        # self.head1 = nn.Linear(dims[-1], num_classes)#分类头1的全连接层
        self.head1 = nn.Linear(dims[-1]*2, num_classes)#分类头1的全连接层
        self.head2 = nn.Linear(dims[-1], num_classes)# 分类头2的全连接层
        self.apply(self._init_weights)#初始化
        self.head1.weight.data.mul_(head_init_scale)
        self.head1.bias.data.mul_(head_init_scale)#新加
        self.head2.weight.data.mul_(head_init_scale)
        self.head2.bias.data.mul_(head_init_scale)#新加
...
#以下新加
        ...
        return self.norm1(x.mean([-2, -1])),self.norm2(l[0].mean([-2, -1]))
#以上新加
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x0,out0 = self.forward_features(x)#new
        x = self.head1(x0)
        out = self.head2(out0)


        return x,out,x0,out0
